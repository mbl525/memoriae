/* 
This code is also based upon WADD W8L1 example on Storing and Displaying Data Locally, focusing on displaying data
Author: ace01603
...
document.getElementById("next-page").addEventListener("click", nextConcert);
document.getElementById("prev-page").addEventListener("click", previousConcert);
...
function displayConcert() {
    ...
    leftPage.innerHTML =
    `<h1> ${concert.tourName}</h1> <p>${concert.notes}</p>`;
    rightPage.innerHTML =
    `<div class="info-card">
    ...
    ${concert.polImg.map((image, index) =>
        `<div class="polaroid photo-${index + 1}">
            <img src="${image}" class="concert-photo" alt="Your Concert Photos">
        </div>`
    ).join("")}
    </div>`;
}
